...
function build-rust-worker() {
	case "$TARGET" in
		android-arm64)
			# device
			cargo ndk -t arm64-v8a --platform 33 -o ${PROJECTDIR}/android/app/src/main/jniLibs build
			;;
		android-x64)
			# emulator 
			cargo ndk -t x86_64 --platform 33 -o ${PROJECTDIR}/android/app/src/main/jniLibs build
			;;
		linux)
			cargo build
			DESTDIR=${PROJECTDIR}/build/rust
			mkdir -p ${DESTDIR}
			cratename=$(grep name "Cargo.toml" | cut -f2 -d\")
			cp -v ${CARGO_TARGET_DIR}/${MODE}/lib${cratename}.so ${DESTDIR}
			;;
		*)
