...
void
move_to_first_empty(const bin (&bins)[NTICKS], int* k)
{
...
void
move_to_first_non_empty(const bin (&bins)[NTICKS], int* k)
{
  for (; *k < NTICKS && bins[*k].empty(); ++*k)
    ;
}
...
bool
noise_at_index(const bin (&bins)[NTICKS], int k, const int secondsUntilAloneTick, const int minAloneTicks)
{
...
bin
counter::getbin(const int& k) const
{
  assert(0 <= k && k < NTICKS);
...
bool
packed::operator==(const packed& other) const
{
  for (int k = 0; k < NTICKS; ++k) {
...
int
some_spurious_ticks(counter& C, const int minAloneTicks)
{
  int k = 0;
