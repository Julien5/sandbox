...
void
turnLED(uint8_t pin, uint8_t mode)
{
#ifdef ARDUINO
  pinMode(pin, OUTPUT);
...
void
update_display_wifi(wifi::interface& esp)
{
  char line2[17] = { 0 };
...
char
try_upload_statistics(wifi::interface& esp)
{
  uint16_t length = 0;
...
bool
upload_statistics(wifi::interface& esp)
{
  if (counter.empty())
    return true;
...
int
test_upload(wifi::interface& esp)
{
