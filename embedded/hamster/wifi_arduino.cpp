...
unsigned char
waitFor(const unsigned char opts, const int timeout)
{
  /*
...
unsigned char
sendCommandAndWaitForResponse(const char* command, const int length, const int timeout)
{
...
unsigned char
sendCommandAndWaitForResponse(const char* command, const int timeout)
{
...
wifi::esp8266::join()
{
  ...
  if (!m_joined) {
    ...
    //  delay(250);
    //}
    m_joined = sendCommandAndWaitForResponse("AT+CWJAP_CUR=\"JBO\",\"7981409790562366\"", long_timeout) == options::wait_for_ok;
  }
  return m_joined;
}
...
class IPConnection
{
  ...
  bool open()
  {
    ...
    if ((millis() - last_connection_time) < 1000)
      delay(1000);
    return (sendCommandAndWaitForResponse("AT+CIPSTART=\"TCP\",\"pi.fritz.box\",8000", long_timeout) & options::wait_for_ok) != 0;
  }
  bool close()
  {
    last_connection_time = millis();
    return (sendCommandAndWaitForResponse(ATCIPCLOSE, short_timeout) & options::wait_for_ok) != 0;
  }
...
bool
wifi::esp8266::get(const char* req, char** response)
{
...
int
wifi::esp8266::post(const char* req, const uint8_t* data, const int Ldata, char** response)
{
  //while (!ping()) {
